...
import { Addon, OrderItemsDropdownProps } from "@/src/interfaces/interfaces";
        ...
        <div className="flex flex-col bg-gray-100 rounded-lg p-3 gap-4">
            ...
            >
                <p className="flex flex-row gap-1">
                    <strong className="font-light text-[#00000080]">24 يناير, 2025</strong>
                </p>
                <p className="flex flex-row gap-4 items-center">
                    <strong>4 أصناف</strong>
                    {isOpen ? (
                        <ChevronUpIcon />
                    ) : (
                        <ChevronDownIcon />
                    ...
                    {items.map((item, index) => (
                        <li key={index} className="flex justify-between flex-col text-[12px]">
                            ...
                            <ul className="flex flex-col px-6 justify-between font-light gap-[2px]">
                                {item.addons.map(
                                    (addon, index) => (
                                        <li key={index} className="flex flex-row justify-between w-full">
                                            <p className="flex flex-row gap-2">
                                                <span>
                                                    X{addon.number}
                                                </span>
