...
const PREIDEN: Record<number,string> = {1: 'dine_', 2: ''}

export default function CachedMenuContainer({ lang, brnid, type }: CachedMenuContainerProps) {
  ...
  const IDEN = PREIDEN[type]

  const CATEGORIES_CACHE_KEY = `${IDEN}menu_categories_${brnid}_${lang}`;
  const CATEGORIES_CACHE_EXPIRY_KEY = `${IDEN}menu_categories_expiry_${brnid}_${lang}`;
  const ITEMS_CACHE_PREFIX = `${IDEN}menu_items_${brnid}_${lang}_`;
  const ITEMS_CACHE_EXPIRY_PREFIX = `${IDEN}menu_items_expiry_${brnid}_${lang}_`;
